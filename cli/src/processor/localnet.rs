...
use {
    ...
    clap::crate_version,
    open_clockwork_network_program::state::{Config, ConfigSettings, Registry},
    open_clockwork_thread_program::state::{Thread, Trigger},
    solana_sdk::{
...
fn create_threads(client: &Client, mint_pubkey: Pubkey) -> Result<()> {
    ...
    let ix_a1 = Instruction {
        ...
        accounts: open_clockwork_network_program::accounts::DistributeFeesJob {
            config: Config::pubkey(),
            registry: Registry::pubkey(),
            thread: epoch_thread_pubkey,
        }
        .to_account_metas(Some(false)),
        data: open_clockwork_network_program::instruction::DistributeFeesJob {}.data(),
    };
    let ix_a2 = Instruction {
        program_id: open_clockwork_network_program::ID,
        accounts: open_clockwork_network_program::accounts::ProcessUnstakesJob {
            config: Config::pubkey(),
            registry: Registry::pubkey(),
            thread: epoch_thread_pubkey,
        }
        .to_account_metas(Some(false)),
        data: open_clockwork_network_program::instruction::ProcessUnstakesJob {}.data(),
    };
    let ix_a3 = Instruction {
        program_id: open_clockwork_network_program::ID,
        accounts: open_clockwork_network_program::accounts::StakeDelegationsJob {
            config: Config::pubkey(),
            registry: Registry::pubkey(),
            thread: epoch_thread_pubkey,
        }
        .to_account_metas(Some(false)),
        data: open_clockwork_network_program::instruction::StakeDelegationsJob {}.data(),
    };
    let ix_a4 = Instruction {
        program_id: open_clockwork_network_program::ID,
        accounts: open_clockwork_network_program::accounts::TakeSnapshotJob {
            config: Config::pubkey(),
            registry: Registry::pubkey(),
            thread: epoch_thread_pubkey,
        }
        .to_account_metas(Some(false)),
        data: open_clockwork_network_program::instruction::TakeSnapshotJob {}.data(),
    };
    let ix_a5 = Instruction {
        program_id: open_clockwork_network_program::ID,
        accounts: open_clockwork_network_program::accounts::EpochCutover {
            config: Config::pubkey(),
            registry: Registry::pubkey(),
            thread: epoch_thread_pubkey,
        }
        .to_account_metas(Some(false)),
        data: open_clockwork_network_program::instruction::IncrementEpoch {}.data(),
    };
    let ix_a6 = Instruction {
        program_id: open_clockwork_network_program::ID,
        accounts: open_clockwork_network_program::accounts::DeleteSnapshotJob {
            config: Config::pubkey(),
            registry: Registry::pubkey(),
            thread: epoch_thread_pubkey,
        }
        .to_account_metas(Some(false)),
        data: open_clockwork_network_program::instruction::DeleteSnapshotJob {}.data(),
    };
    let ix_a = Instruction {
        program_id: open_clockwork_thread_program::ID,
        accounts: open_clockwork_thread_program::accounts::ThreadCreate {
        ...
        .to_account_metas(Some(false)),
        data: open_clockwork_thread_program::instruction::ThreadCreate {
    ...
    let registry_hash_ix = Instruction {
        program_id: open_clockwork_network_program::ID,
        accounts: open_clockwork_network_program::accounts::RegistryNonceHash {
            config: Config::pubkey(),
            registry: Registry::pubkey(),
            thread: hasher_thread_pubkey,
        }
        .to_account_metas(Some(false)),
        data: open_clockwork_network_program::instruction::RegistryNonceHash {}.data(),
    };
...
fn create_geyser_plugin_config(config: &CliConfig) -> Result<()> {
    let geyser_config = open_clockwork_plugin_utils::PluginConfig {
...
fn start_test_validator(
    ...
    cmd.arg("-r")
        .bpf_program(config, open_clockwork_network_program::ID, "network")
        .bpf_program(config, open_clockwork_thread_program::ID, "thread")
        .bpf_program(config, open_clockwork_webhook_program::ID, "webhook")
...
impl TestValidatorHelpers for Command {
    ...
    fn bpf_program(
        ...
        let filename = format!("open_clockwork_{}_program.so", program_name);
        self.arg("--bpf-program")
